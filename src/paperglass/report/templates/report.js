for (const r of document.querySelectorAll('.region')) {
  ...
  const place = (e) => { const b = r.getBoundingClientRect(); const x = e.clientX - b.left, y = e.clientY - b.top;
    reads.style.clipPath = 'circle(' + (r.classList.contains('page') ? 96 : 80) + 'px at ' + x + 'px ' + y + 'px)'; ring.style.left = x + 'px'; ring.style.top = y + 'px'; r.dataset.lens = 'on'; };
  ...
  for (const hit of r.querySelectorAll('.hit')) hit.addEventListener('pointermove', (e) => e.stopPropagation());
  let t = null;
