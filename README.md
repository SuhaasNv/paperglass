...
**Status (22 Sep 2026):** v0.1.0 (UC1 Scan and Verdict) is built on the `dev` branch and waits for its release pull request. PDF and DOCX inputs, 18 techniques, the CLI and the Python API exist and are tested on Linux, macOS and Windows. The HTML report (v0.2.0), the benchmark (v0.3.0), the adapters (v0.4.0) and the Red Kit (v0.5.0) follow; `SCOPE.md` says what is built and what is not.
