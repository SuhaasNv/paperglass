...
- Web tier (v0.2.0): FastAPI + SQLAlchemy 2 + Alembic + PostgreSQL; React 19 + TypeScript strict + Vite + Tailwind + TanStack Query; `backend/` and `frontend/` directories beside `src/paperglass`; the backend imports the library through `paperglass.engine` only. Uploaded files are scanned in memory and never stored; reports are stored under an unguessable id for 7 days; an anonymous session cookie gives a browser its history; no accounts; no telemetry. Frontend rules: TypeScript strict, no `any`, every status a label plus a shape, no em dashes or emoji in copy, layouts verified at 375, 768 and 1280 px by the Playwright journey, screens built to the design the owner settles with Fable (`docs/04-report-design/`), never invented by Claude.
