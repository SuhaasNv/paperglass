...
Paperglass is an open-source (Apache-2.0) document trust scanner and benchmark for AI pipelines, shipped as a full-stack product: a web app and API (FastAPI, PostgreSQL, React), a Python library and a CLI. It shows what a model will read (View A), compares it with what a human sees (View B) and with document structure (View C), and returns a verdict with evidence. It is the owner's own project, built to be useful to the people who ship and review documents through AI systems. It is not coursework or a graded exercise, and no repository document describes it as one (`scripts/check_copy.py` enforces the wording).
...
- Web tier (v0.2.0): FastAPI + SQLAlchemy 2 + Alembic + PostgreSQL; React 19 + TypeScript strict + Vite + Tailwind + TanStack Query; `backend/` and `frontend/` directories beside `src/paperglass`; the backend imports the library through `paperglass.engine` and the writers in `paperglass.report` only. Uploaded files are scanned in memory and never stored; reports are stored under an unguessable id for 7 days; an anonymous session cookie gives a browser its history; no accounts; no telemetry. Frontend rules: TypeScript strict, no `any`, every status a label plus a shape, no em dashes or emoji in copy, layouts verified at 375, 768 and 1280 px by the Playwright journey, screens built to the design the owner settles with Fable (`docs/04-report-design/`), never invented by Claude.
...
| v0.2.0 | UC2 Web app and report | 5 | FastAPI + PostgreSQL backend (`/api/v1/scans`), React + Vite frontend (upload, results with the word-aligned diff, finding cards, fingerprint, history, techniques), downloadable one-file HTML report, profiles, Docker images on GHCR, Railway development and production, Playwright journey; the 30-second demo; Show HN |
...
4. Local gate before every commit: `uv run ruff check .`, `uv run ruff format --check .`, `uv run mypy --strict`, `uv run pytest` with `--cov-fail-under=90` on detectors, the layering test and golden files; the fuzz smoke set when a parser was touched; the benchmark on the affected corpus with the false-positive rate captured when a detector was touched; for the frontend `npm run lint`, `npm run typecheck`, `npm run test`, and `npm run e2e` when a screen changed. All exit 0 or nothing is committed.
...
- Types: Python 3.11+, `mypy --strict`, `ruff` clean, no `Any` in public signatures, no `# type: ignore` without an error code and a reason, no bare `except`, pydantic models for every schema. Frontend: TypeScript strict, ESLint and Prettier clean, vitest for components and hooks, Playwright for the one journey (upload, results, history).
